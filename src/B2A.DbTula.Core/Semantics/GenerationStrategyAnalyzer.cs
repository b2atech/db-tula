...
public static class GenerationStrategyAnalyzer
{
        ...
        if (targetSync is { IsBehind: true } && verdict.Category < DriftCategory.Functional)
        {
            return new GenerationDrift
            {
                ...
                MigrationScript =
                    ...
                    $"(SELECT COALESCE(MAX(\"{col}\"), 1) FROM {QuoteQualified(table)}));"
            };
        }
    ...
    private static string AddIdentityScript(string table, string col) =>
        $"ALTER TABLE {QuoteQualified(table)} ALTER COLUMN \"{col}\" ADD GENERATED BY DEFAULT AS IDENTITY;\n" +
        $"-- align the new identity sequence with existing data:\n" +
        $"SELECT setval(pg_get_serial_sequence('{QuoteQualified(table)}', '{col}'), " +
        $"(SELECT COALESCE(MAX(\"{col}\"), 1) FROM {QuoteQualified(table)}));";
    ...
    private static string DropIdentityScript(string table, string col) =>
        ...
        $"ALTER TABLE {QuoteQualified(table)} ALTER COLUMN \"{col}\" DROP IDENTITY IF EXISTS;";
    ...
    private static string AlignIdentityKindScript(string table, string col, GenerationStrategy source) =>
        source == GenerationStrategy.IdentityAlways
            ? $"ALTER TABLE {QuoteQualified(table)} ALTER COLUMN \"{col}\" SET GENERATED ALWAYS;"
            : $"ALTER TABLE {QuoteQualified(table)} ALTER COLUMN \"{col}\" SET GENERATED BY DEFAULT;";

    // "public.product_brokerages" wrapped as one quoted identifier is a table literally
    // named "public.product_brokerages" in the search_path schema — not schema.table —
    // which Postgres reports as "relation does not exist". Quote each part separately.
    private static string QuoteQualified(string table)
    {
